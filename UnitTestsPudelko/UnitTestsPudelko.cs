...
namespace PudelkoUnitTests
{
    ...
    public class UnitTestsPudelkoConstructors
    {
        ...
        [DataTestMethod, TestCategory("Properties")]
        [DataRow(1d, 2d, 3d, 6d, 22d)]
        [DataRow(6.3, 0.2, 7.3, 9.1980, 97.42)]
        [DataRow(0.012, 0.044, 0.134, 0.000071, 0.016064)]
        [DataRow(2, 2, 2, 8, 24)]
        public void Pole_Objetosc(double a, double b, double c, double obj, double pole)
        {
            Pudelko p = new Pudelko(a, b, c);
            Assert.AreEqual(pole, p.Pole, delta: accuracy);
            Assert.AreEqual(obj, p.Objetosc, delta: accuracy);
        }
        #endregion
        ...
        public void OperatorEquals(double[] a, double[] b)
        {
            var p1 = new Pudelko(a[0], a[1], a[2]);
            var p2 = new Pudelko(b[0], b[1], b[2]);
            Assert.IsTrue(p1 == p2);
        }
        ...
        public void OperatorNotEquals(double[] a, double[] b)
        {
            var p1 = new Pudelko(a[0], a[1], a[2]);
            var p2 = new Pudelko(b[0], b[1], b[2]);
        ...
        public void OperatorPlus(double[] a, double[] b, double[] c)
        {
            var p1 = new Pudelko(a[0], a[1], a[2]);
            var p2 = new Pudelko(b[0], b[1], b[2]);
            ...
            Assert.AreEqual(c[0], p.A, delta: accuracy);
            Assert.AreEqual(c[1], p.B, delta: accuracy);
            Assert.AreEqual(c[2], p.C, delta: accuracy);
        }
        #endregion
        ...
        [DataRow("2.500 m × 9.321 m × 0.100 m", 2.5, 9.321, 0.100)]
        [DataRow("2.646 cm × 4 mm × 2 m", 0.02646, 0.004, 2)]
        [DataRow("30 cm × 300 cm × 60 cm", 0.3, 3, 0.6)]
        public void Parsing(string text, double a, double b, double c)
        {
            Pudelko p = Pudelko.Parse(text);

            Assert.AreEqual(a, p.A);
            Assert.AreEqual(b, p.B);
            Assert.AreEqual(c, p.C);
        }
        #endregion
